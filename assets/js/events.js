document.addEventListener('DOMContentLoaded', function () {
    ...
    window.togglePricing(false);
            ...
            // Reset Gallery Previews
            document.querySelectorAll('.gallery-slot').forEach(slot => {
                slot.querySelector('.gallery-preview').style.display = 'none';
                slot.querySelector('.plus-icon').style.display = 'block';
                slot.querySelector('.remove-gallery-img').style.display = 'none';
                slot.style.borderStyle = 'dashed';
                slot.style.borderColor = '#eee';
            });
        };

        if (hostEventBtn) hostEventBtn.onclick = window.openEventModal;
        ...
        window.editEvent = async function (id) {
            try {
                ...
                if (e.image_path) {
                    const formattedImgPath = e.image_path.startsWith('images/') ? '../' + e.image_path : e.image_path;
                    const previewImg = document.getElementById('preview_img');
                ...
                if (e.gallery_images) {
                    const existingGallery = e.gallery_images.split(',');
                    document.querySelectorAll('.gallery-slot').forEach((slot, idx) => {
                        const preview = slot.querySelector('.gallery-preview');
                        const plus = slot.querySelector('.plus-icon');
                        const removeBtn = slot.querySelector('.remove-gallery-img');
                        if (existingGallery[idx] && preview && plus && removeBtn) {
                            const formattedGalleryImg = existingGallery[idx].trim().startsWith('images/')
                                ? '../' + existingGallery[idx].trim()
                                : existingGallery[idx].trim();
                            preview.src = formattedGalleryImg;
                            preview.style.display = 'block';
                            plus.style.display = 'none';
                            removeBtn.style.display = 'block';
                            slot.style.borderStyle = 'solid';
    ...
    function renderEvents(events) {
        ...
        if (currentState.view === 'my') {
            html += `
                <div class="event-card host-event-btn" onclick="window.openEventModal()" style="height: 100%; min-height: 400px; display: flex; flex-direction: column; justify-content: center; align-items: center; border: 2px dashed #f5a623; border-radius: 20px; background: #fffcf5; cursor: pointer; transition: all 0.3s; transform-origin: center;">
        ...
        return items.map((e, index) => {
            ...
            <div class="event-card" style="border: ${isPremium ? '2px solid #f5a623' : '1px solid #f0f0f0'}; border-radius: 16px; overflow: hidden; background: white; display: flex; flex-direction: column; position: relative; transition: transform 0.3s ease;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'">
                ...
                <a href="event-detail.php?id=${e.id}" style="display: block; position: relative; height: 180px;">
                    <img src="${e.image_path.startsWith('images/') ? '../' + e.image_path : e.image_path}" onerror="this.src='../images/placeholder_event.jpg'" style="width: 100%; height: 100%; object-fit: cover;">
    ...
    function renderFeaturedSidebar(premiumEvents) {
        ...
        grid.innerHTML = premiumEvents.slice(0, 3).map(e => `
            <a href="event-detail.php?id=${e.id}" style="text-decoration: none; display: flex; gap: 15px; align-items: center; background: white; padding: 12px; border-radius: 12px; border: 1px solid #eee; margin-bottom: 12px;">
                <img src="${e.image_path.startsWith('images/') ? '../' + e.image_path : e.image_path}" onerror="this.src='../images/placeholder_event.jpg'" style="width: 60px; height: 60px; border-radius: 8px; object-fit: cover;">
                <div style="flex: 1;">
