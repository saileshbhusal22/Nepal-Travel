...
function openTicketModal(e) {
    ...
    if (isPaid) {
        ...
        document.getElementById('modalSubmitBtn').setAttribute('onclick', "document.getElementById('ticketBookingForm').action='event_esewa_booking_initiate.php'");
        
        const khaltiBtn = document.getElementById('modalKhaltiBtn');
        if(khaltiBtn) khaltiBtn.style.display = 'flex';
    } else {
        ...
        const khaltiBtn = document.getElementById('modalKhaltiBtn');
        if(khaltiBtn) khaltiBtn.style.display = 'none';
    }
...
function getCategoryFallbackImage(category, eventId = 0) {
    ...
    const pools = {
        FESTIVAL: ['images/phewa_sunset.png', 'images/pokhara_lake.png', 'images/family_fun_nepal.png'],
        CONCERT: ['images/kathmandu_night_hero.png', 'images/city_excitement_nepal.png'],
        WORKSHOP: ['images/city_excitement_nepal.png', 'images/ktm_durbar.png'],
        FOOD: ['images/food_drinks_nepal.png', 'images/family_fun_nepal.png'],
        SPORTS: ['images/annapurna_trek.png', 'images/everest_trek.png'],
        NATURE: ['images/chitwan_rhino.png', 'images/ebc_summit.png'],
        ARTS: ['images/bhaktapur_temple.png', 'images/pashupatinath_aarti.png'],
        'ARTS & CULTURE': ['images/bhaktapur_temple.png', 'images/pashupatinath_aarti.png'],
        RELIGIOUS: ['images/pashupatinath_aarti.png', 'images/lumbini_temple.png'],
        NIGHTLIFE: ['images/kathmandu_night_hero.png'],
        PHOTOGRAPHY: ['images/sarangkot_sunrise.png', 'images/annapurna_trek.png'],
    };
    const pool = pools[cat] || ['images/pokhara_lake.png', 'images/bhaktapur_temple.png', 'images/chitwan_rhino.png'];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Hero
            const imgPath = resolveEventImageUrl(getEventCoverPath(e));
            const dImg = document.getElementById('dImg');
            dImg.src = imgPath;
            dImg.onerror = function () {
                this.onerror = null;
                this.src = resolveEventImageUrl(getCategoryFallbackImage(e.category, e.id));
            };
            ...
            if (e.gallery_images && e.gallery_images.trim() !== '') {
                galleryList = e.gallery_images.split(',')
                    .map((img) => img.trim())
                    .filter(Boolean)
                    .map((img) => resolveEventImageUrl(img));
            }
            ...
            const gHtml = galleryList.map((src, i) => `
                <div class="gallery-item ${i === 0 ? 'wide' : ''}">
                    <img src="${src}" alt="Event photo ${i + 1}" onerror="this.src='../images/placeholder_event.jpg'">
                </div>
            `).join('');
    ...
    try {
        ...
        if (data.success) {
            ...
            grid.innerHTML = filtered.map(ev => {
                const relatedImg = resolveEventImageUrl(ev.display_image || getEventCoverPath(ev));
    ...
    <div class="modal-card">
        ...
        <form id="ticketBookingForm" method="POST" style="display: none;" action="event_esewa_booking_initiate.php">
            ...
            <div class="modal-actions" style="display: flex; gap: 10px; flex-direction: column;">
                <button type="submit" id="modalSubmitBtn" class="modal-btn primary" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px;" onclick="document.getElementById('ticketBookingForm').action='event_esewa_booking_initiate.php'">
                    <span class="esewa-btn-logo">e</span> Pay with eSewa
                </button>
                <button type="submit" id="modalKhaltiBtn" class="modal-btn primary" style="width: 100%; display: none; align-items: center; justify-content: center; gap: 10px; background: #5C2D91; color: white; border: none;" onclick="document.getElementById('ticketBookingForm').action='khalti_event_booking_initiate.php'">
                    <span style="font-weight: 900;">K</span> Pay with Khalti
                </button>
            </div>
        </form>
    </div>
