...
if (!isset($_SESSION['user_id'])) {
    header('Location: /Nepal-Travel/user/login.php?redirect=subscription'); exit;
}
$user_id = (int)$_SESSION['user_id'];

// ── Auto-expire user_deals past visible_until ──────────────────
$conn->query("
    UPDATE user_deals
    SET status = 'expired'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'subscribe') {
    ...
    $plan_row = $conn->query("SELECT * FROM subscription_plans WHERE id=$plan_id AND is_active=1");
    if ($plan_row && $plan_row->num_rows > 0) {
        ...
        if ($conn->query($sql)) {
            header('Location: subscription.php?msg=submitted'); exit;
        } else {
...
// ── Handle: Submit deal ────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'submit_deal') {
    $sub_id      = (int)$_POST['subscription_id'];
    $title       = $conn->real_escape_string(trim($_POST['title']          ?? ''));
    $emoji       = $conn->real_escape_string(trim($_POST['emoji']          ?? '🏔️'));
    $category    = $conn->real_escape_string(trim($_POST['category']       ?? ''));
    $location    = $conn->real_escape_string(trim($_POST['location']       ?? ''));
    $days        = (int)($_POST['days']                                    ?? 0);
    $season      = $conn->real_escape_string(trim($_POST['season']         ?? ''));
    $price       = (float)($_POST['price']                                 ?? 0);
    $orig_price  = (float)($_POST['original_price']                        ?? 0);
    $features    = $conn->real_escape_string(trim($_POST['features']       ?? ''));
    $description = $conn->real_escape_string(trim($_POST['description']    ?? ''));
    $img1        = $conn->real_escape_string(trim($_POST['image_url']      ?? ''));
    $img2        = $conn->real_escape_string(trim($_POST['image_url_2']    ?? ''));
    $img3        = $conn->real_escape_string(trim($_POST['image_url_3']    ?? ''));
    $img4        = $conn->real_escape_string(trim($_POST['image_url_4']    ?? ''));

    $sub_check = $conn->query("
        SELECT us.*, sp.deal_limit, sp.duration_days
    ...
    if (!$sub_check || $sub_check->num_rows === 0) {
        $msg = 'Invalid or inactive subscription.'; $msg_type = 'error';
    } elseif (empty($title) || $price <= 0) {
        $msg = 'Title and Price are required.'; $msg_type = 'error';
    } else {
        $sub  = $sub_check->fetch_assoc();
        $used = (int)$conn->query(
            "SELECT COUNT(*) AS c FROM user_deals WHERE subscription_id=$sub_id AND status != 'expired'"
        )->fetch_assoc()['c'];

        if ($used >= (int)$sub['deal_limit']) {
            $msg = 'You have reached the deal limit for this subscription.'; $msg_type = 'error';
        } else {
            $sql = "INSERT INTO user_deals
                        (user_id, subscription_id, title, emoji, category, location, days, season,
                         price, original_price, features, description,
                         image_url, image_url_2, image_url_3, image_url_4, status)
                    VALUES ($user_id, $sub_id, '$title', '$emoji', '$category', '$location', $days,
                            '$season', $price, $orig_price, '$features', '$description',
                            '$img1', '$img2', '$img3', '$img4', 'pending')";
            if ($conn->query($sql)) {
                $conn->query("UPDATE user_subscriptions SET deals_posted = deals_posted + 1 WHERE id = $sub_id");
                header('Location: subscription.php?msg=deal_submitted'); exit;
            } else {
                $msg = 'Database error: ' . $conn->error; $msg_type = 'error';
            }
        }
...
$plans_result = $conn->query("SELECT * FROM subscription_plans WHERE is_active=1 ORDER BY price ASC");
$plans        = $plans_result ? $plans_result->fetch_all(MYSQLI_ASSOC) : [];

$my_subs = $conn->query("
    SELECT us.*, sp.name AS plan_name, sp.display_name, sp.deal_limit
    FROM user_subscriptions us
    JOIN subscription_plans sp ON sp.id = us.plan_id
...
$my_deals = $conn->query("
    SELECT ud.*, us.status AS sub_status, sp.display_name AS plan_display
    FROM user_deals ud
...
$plan_icons  = ['pay_per_deal' => '⚡', 'monthly' => '📅', 'yearly' => '🏆'];
$plan_colors = ['pay_per_deal' => '#e84393', 'monthly' => '#2563eb', 'yearly' => '#c9a227'];
...
<title>List Your Deal | Nepal Tours</title>
...
.ds-pending{background:rgba(201,162,39,.12);color:var(--gold);border:1px solid rgba(201,162,39,.2)}
.ds-approved{background:rgba(76,175,125,.12);color:var(--green);border:1px solid rgba(76,175,125,.2)}
.ds-rejected{background:rgba(224,85,85,.12);color:var(--red);border:1px solid rgba(224,85,85,.2)}
.ds-expired{background:rgba(255,255,255,.04);color:var(--muted2);border:1px solid var(--border)}
...
/* ── DEAL FORM MODAL ── */
.deal-form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
...
<div class="sub-hero">
  ...
  <div style="position:relative;z-index:1">
    <div class="sub-hero-tag">// List Your Business</div>
    <h1>Promote Your<br><em>Nepal Travel Deal</em></h1>
    <p>Choose a plan, submit your deal, and reach thousands of travelers exploring Nepal every day.</p>
  </div>
</div>
...
<div class="page-wrap">
    ...
    <div class="plan-card <?= $popular ? 'popular' : '' ?>">
      ...
      <div class="plan-price-row">
        ...
        <div class="plan-price-sub">
          <?php if ($plan['name'] === 'monthly'): ?>/ month
          <?php elseif ($plan['name'] === 'yearly'): ?>/ year
          <?php else: ?>one-time<?php endif; ?>
        </div>
      </div>
      <div class="plan-features">
        <div class="plan-feat">Visible for <?= (int)$plan['duration_days'] ?> days</div>
        <div class="plan-feat">Up to <?= (int)$plan['deal_limit'] ?> deal<?= (int)$plan['deal_limit'] > 1 ? 's' : '' ?></div>
        <div class="plan-feat">Admin-reviewed &amp; published</div>
        <?php if ($plan['name'] !== 'pay_per_deal'): ?>
          <div class="plan-feat">Auto-removed after expiry</div>
        <?php endif; ?>
        <?php if ($plan['name'] === 'yearly'): ?>
      ...
      <button class="plan-cta"
        ...
        style="background:<?= $color ?>;color:<?= $plan['name'] === 'yearly' ? '#000' : '#fff' ?>">
  ...
  <div class="sub-cards">
    <?php foreach ($active_subs as $sub):
      ...
      $used    = (int)$sub['deals_posted'];
      $limit   = (int)$sub['deal_limit'];
    ...
    <div class="sub-card">
      ...
      <?php if ($sub['status'] === 'active' && $used < $limit): ?>
        <button class="btn-submit-deal" onclick="openDealModal(<?= (int)$sub['id'] ?>)">
          + Submit Deal
        </button>
      <?php endif; ?>
    </div>
  ...
  <!-- ── MY DEALS ── -->
  <div class="section-title">My Submitted Deals</div>
  <?php if (!empty($my_deals_arr)): ?>
  <div class="deals-table-wrap">
    <div class="dt-header">
      <div class="dt-header-title">All My Deals</div>
    </div>
    <div class="table-scroll">
      <table>
        <thead>
          <tr>
            <th>Image</th><th>Title</th><th>Plan</th><th>Price (NPR)</th>
            <th>Visible Until</th><th>Status</th><th>Note</th>
          </tr>
        </thead>
        <tbody>
          <?php foreach ($my_deals_arr as $d):
            $hasImg      = !empty($d['image_url']) && strtoupper(trim($d['image_url'])) !== 'NULL';
            $statusClass = 'ds-' . $d['status'];
            $until       = $d['visible_until'] ? date('M d, Y', strtotime($d['visible_until'])) : '—';
          ?>
          <tr>
            <td>
              <?php if ($hasImg): ?>
                <img src="<?= htmlspecialchars($d['image_url']) ?>" class="deal-thumb"
                     onerror="this.style.display='none'" alt="">
              <?php else: ?>
                <div class="deal-thumb-placeholder"><?= !empty($d['emoji']) ? $d['emoji'] : '🏔️' ?></div>
              <?php endif; ?>
            </td>
            ...
            <td><span class="status-pill <?= $statusClass ?>"><?= ucfirst($d['status']) ?></span></td>
            <td style="font-size:12px;color:var(--muted);max-width:160px">
              <?= !empty($d['rejection_reason']) ? htmlspecialchars($d['rejection_reason']) : '—' ?>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
  ...
  <div class="deals-table-wrap">
    <div class="empty-state">
      ...
      <p>No deals submitted yet. Purchase a plan above to get started!</p>
    </div>
  </div>
  <?php endif; ?>
...
<div class="modal-bd" id="dealModal" onclick="closeIfBd(event,'dealModal')">
  <div class="modal-box" style="max-width:600px">
    <div class="modal-hd">
      <div class="modal-hd-title">Submit Your Deal</div>
      <button class="modal-close" onclick="closeModal('dealModal')">✕</button>
    </div>
    <div class="modal-body">
      <form method="POST" action="subscription.php">
        <input type="hidden" name="action"          value="submit_deal">
        <input type="hidden" name="subscription_id" id="deal_sub_id">
        <div class="deal-form-grid">
          <div class="form-group full">
            <label class="flbl">Deal Title *</label>
            <input type="text" name="title" class="fin" placeholder="e.g. Annapurna Circuit 14 Days" required>
          </div>
          <div class="form-group">
            <label class="flbl">Emoji</label>
            <input type="text" name="emoji" class="fin" placeholder="🏔️" value="🏔️">
          </div>
          <div class="form-group">
            <label class="flbl">Category</label>
            <input type="text" name="category" class="fin" placeholder="e.g. Trekking">
          </div>
          <div class="form-group">
            <label class="flbl">Location</label>
            <input type="text" name="location" class="fin" placeholder="e.g. Annapurna Region">
          </div>
          <div class="form-group">
            <label class="flbl">Season</label>
            <input type="text" name="season" class="fin" placeholder="e.g. Spring / Autumn">
          </div>
          <div class="form-group">
            <label class="flbl">Price (NPR) *</label>
            <input type="number" name="price" class="fin" placeholder="25000" required step="1">
          </div>
          <div class="form-group">
            <label class="flbl">Original Price (NPR)</label>
            <input type="number" name="original_price" class="fin" placeholder="0" step="1">
            <div class="fin-hint">Leave blank if no discount</div>
          </div>
          <div class="form-group">
            <label class="flbl">Duration (Days)</label>
            <input type="number" name="days" class="fin" placeholder="14" min="1">
          </div>
          <div class="form-group full">
            <label class="flbl">Features / Highlights</label>
            <input type="text" name="features" class="fin" placeholder="Guided Tour, Meals, Permits, Porter">
            <div class="fin-hint">Separate with commas</div>
          </div>
          <div class="form-group full">
            <label class="flbl">Description</label>
            <textarea name="description" class="fin" placeholder="Describe your deal, itinerary, what's included…"></textarea>
          </div>
          <div class="form-group full">
            <label class="flbl">Main Image URL</label>
            <input type="text" name="image_url" class="fin" placeholder="https://…">
          </div>
          <div class="form-group">
            <label class="flbl">Image 2 URL</label>
            <input type="text" name="image_url_2" class="fin" placeholder="https://…">
          </div>
          <div class="form-group">
            <label class="flbl">Image 3 URL</label>
            <input type="text" name="image_url_3" class="fin" placeholder="https://…">
          </div>
        </div>
        <button type="submit" class="btn-submit btn-manual" style="margin-top:8px">Submit for Review →</button>
      </form>
    </div>
  </div>
</div>
...
function openDealModal(subId) {
  document.getElementById('deal_sub_id').value = subId;
  openModal('dealModal');
}
