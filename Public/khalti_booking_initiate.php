...
// ════════════════════════════════════════════════════════════════
$transaction_uuid = 'BOOK-KHALTI-' . uniqid() . '-' . time();

if (!isset($_SESSION['pending_khalti_bookings'])) {
    $_SESSION['pending_khalti_bookings'] = [];
}
$_SESSION['pending_khalti_bookings'][$transaction_uuid] = [
    'user_id'     => $user_id,
    'name'        => $name,
...
if ($http_code === 200 && !empty($result['payment_url'])) {
    ...
    // ── API error — remove from session and go back ────────
    if (isset($_SESSION['pending_khalti_bookings'][$transaction_uuid])) {
