<?php
/**
 * khalti_subscription_initiate.php
 * Initiates a Khalti payment gateway v2 transaction for buying/renewing a subscription plan.
 */
session_start();
...
// Duplicate checks: block if user already has an active or pending subscription for this plan.
// Note: We allow multiple active "Pay Per Deal" (plan_id = 1) purchases.
if ($plan_id === 1) {
    // Only block if there is a pending subscription for Pay Per Deal (preventing double-checkout)
    ...
    // For Monthly/Yearly plans, block if there is a pending or active subscription
    $dupStmt = $conn->prepare(
        "SELECT id FROM user_subscriptions
...
$transaction_uuid = 'SUB-KHALTI-' . uniqid() . '-' . time();

// Keep pending subscription details in session
if (!isset($_SESSION['pending_khalti_subs'])) {
    $_SESSION['pending_khalti_subs'] = [];
}
$_SESSION['pending_khalti_subs'][$transaction_uuid] = [
    'user_id' => $user_id,
    'plan_id' => $plan_id,
...
$post_fields = [
    'return_url' => SITE_URL . '/khalti_subscription_callback.php',
    'website_url' => 'http://localhost/Nepal-Travel',
    'amount' => $amount_paisa,
    'purchase_order_id' => $transaction_uuid,
    'purchase_order_name' => 'Subscription Plan - ' . $plan['display_name'],
    'customer_info' => [
        'name' => $customer_name,
...
if ($curl_error) {
    error_log("[Khalti Sub Initiate] cURL error: " . $curl_error);
    header('Location: subscription.php?esewa_error=' . urlencode('Failed to connect to Khalti payment gateway.'));
    exit;
}

$result = json_decode($response, true);
error_log("[Khalti Sub Initiate] HTTP $http_code Response: " . $response);
...
if ($http_code === 200 && isset($result['payment_url'])) {
    ...
    error_log("[Khalti Sub Initiate] Error payload: " . json_encode($result));
    header('Location: subscription.php?esewa_error=' . urlencode($error_msg));
    exit;
}
