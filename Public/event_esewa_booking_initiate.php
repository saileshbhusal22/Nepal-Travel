...
if ($amount <= 0.0) {
    ...
    // PAID FLOW: Generate Pending Booking and Redirect to eSewa
    $stmt = $conn->prepare("INSERT INTO bookings (user_id, name, destination, date, guests, status, payment_method, amount, event_id, created_at) VALUES (?, ?, ?, ?, ?, 'pending', 'esewa', ?, ?, NOW())");
    $stmt->bind_param("isssidi", $user_id, $name, $title, $travel_date, $guests, $amount, $event_id);
    
    if (!$stmt->execute()) {
        die("Error initiating booking: " . $stmt->error);
    }
    
    $booking_id = $stmt->insert_id;
    $stmt->close();
    
    // Construct eSewa payload parameters
    $transaction_uuid = "BOOK-" . $booking_id . "-" . time();
    
    // Save transaction code reference in database
    $up = $conn->prepare("UPDATE bookings SET payment_ref = ? WHERE id = ?");
    $up->bind_param("si", $transaction_uuid, $booking_id);
    $up->execute();
    $up->close();
