...
// ── Get ID from URL ──────────────────────────────────────────
$ud_id = isset($_GET['ud']) ? (int)$_GET['ud'] : 0;
if ($ud_id <= 0) { header("Location: deals.php"); exit; }
...
// ── Fetch reviews from user_deal_reviews ────────────────────
$reviews = [];
...
// All 4 image slots
$allImages = [];
foreach (['image_url','image_url_2','image_url_3','image_url_4'] as $key) {
    if (!empty($deal[$key]) && strtoupper(trim($deal[$key])) !== 'NULL') {
        $allImages[] = $deal[$key];
    }
}
...
// Expiry countdown
$daysLeft = null;
if (!empty($deal['visible_until'])) {
    $daysLeft = max(0, (int)ceil((strtotime($deal['visible_until']) - time()) / 86400));
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero-img { width: 100%; height: 100%; object-fit: cover; display: block; transform: scale(1.05); transition: transform 8s ease; }
    .hero-img.loaded { transform: scale(1); }
    .hero-fallback { width: 100%; height: 100%; background: linear-gradient(135deg, #1a1f35 0%, #0d1020 100%); display: flex; align-items: center; justify-content: center; font-size: 130px; }
    .hero-grad { position: absolute; inset: 0; background: linear-gradient(to bottom, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.45) 55%, rgba(0,0,0,0.88) 100%); }
    .hero-top { position: absolute; top: 0; left: 0; right: 0; z-index: 4; display: flex; align-items: center; justify-content: space-between; padding: 1.5rem 2rem; }
    .back-btn { display: inline-flex; align-items: center; gap: 8px; background: rgba(0,0,0,0.40); border: 1px solid rgba(255,255,255,0.20); color: #fff; font-size: 13px; font-weight: 500; padding: 9px 20px; border-radius: 30px; text-decoration: none; backdrop-filter: blur(8px); transition: background 0.2s; }
    .back-btn:hover { background: rgba(0,0,0,0.65); }
    .hero-badges { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
    .badge-cat { background: rgba(92,63,204,0.92); backdrop-filter: blur(6px); color: #fff; font-size: 10px; font-weight: 600; padding: 6px 15px; border-radius: 20px; text-transform: uppercase; letter-spacing: 0.08em; }
    .badge-disc { background: #e84393; color: #fff; font-size: 11px; font-weight: 700; padding: 6px 13px; border-radius: 8px; }
    .badge-partner { background: rgba(201,162,39,0.92); backdrop-filter: blur(6px); color: #000; font-size: 9px; font-weight: 800; letter-spacing: 1.5px; padding: 6px 14px; border-radius: 20px; text-transform: uppercase; }
    .hero-bottom { position: absolute; bottom: 0; left: 0; right: 0; z-index: 3; padding: 0 2.5rem 2.8rem; max-width: 900px; }
    .hero-eyebrow { font-size: 10px; font-weight: 700; letter-spacing: 0.22em; text-transform: uppercase; color: rgba(255,255,255,0.45); margin-bottom: 0.5rem; }
    .hero-title { font-family: 'Playfair Display', serif; font-size: clamp(2rem, 4.5vw, 3.2rem); font-weight: 700; color: #fff; line-height: 1.10; text-shadow: 0 2px 28px rgba(0,0,0,0.55); margin-bottom: 1rem; }
    ...
    .expiry-badge-meta { display: inline-flex; align-items: center; gap: 5px; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
    .expiry-ok   { background: rgba(76,175,125,0.20); color: #4caf7d; border: 1px solid rgba(76,175,125,0.30); }
    .expiry-warn { background: rgba(240,160,48,0.20); color: #f0a030; border: 1px solid rgba(240,160,48,0.35); }
    .expiry-hot  { background: rgba(224,85,85,0.22); color: #e05555; border: 1px solid rgba(224,85,85,0.35); }

    /* rating clickable pill in hero */
    .rating-clickable {
      cursor: pointer;
    ...
    /* ══ GALLERY STRIP ══ */
    .gallery-strip { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 4px; max-height: 200px; overflow: hidden; }
    .gallery-strip img { width: 100%; height: 200px; object-fit: cover; display: block; cursor: pointer; opacity: 0.78; transition: opacity 0.2s, transform 0.3s; }
    ...
    .lb-close { position: absolute; top: 1.5rem; right: 1.5rem; background: rgba(255,255,255,0.12); border: 1px solid rgba(255,255,255,0.2); color: #fff; font-size: 22px; width: 44px; height: 44px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; }
    .lb-close:hover { background: rgba(255,255,255,0.25); }
    .lb-prev, .lb-next { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.18); color: #fff; font-size: 30px; width: 50px; height: 50px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.2s; }
    .lb-prev { left: 1.5rem; } .lb-next { right: 1.5rem; }
    .lb-prev:hover, .lb-next:hover { background: rgba(255,255,255,0.25); }

    /* ══ LAYOUT ══ */
    .detail-layout { max-width: 1240px; margin: 0 auto; padding: 3rem 2rem; display: grid; grid-template-columns: 1fr 360px; gap: 3rem; align-items: start; }
    @media (max-width: 880px) { .detail-layout { grid-template-columns: 1fr; padding: 1.5rem 1rem; } .detail-hero { height: 380px; } .hero-bottom { padding: 0 1.5rem 2rem; } }

    /* ══ GLASS CARDS ══ */
    .glass-card { background: rgba(18,21,31,0.85); backdrop-filter: blur(14px); border: 1px solid rgba(255,255,255,0.07); border-radius: 18px; padding: 1.8rem; margin-bottom: 1.4rem; }
    .glass-card.partner-highlight { border-color: rgba(201,162,39,0.25); }
    .sec-label { font-size: 10px; font-weight: 700; letter-spacing: 0.18em; text-transform: uppercase; color: rgba(255,255,255,0.30); margin-bottom: 1rem; }
    .stat-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(105px, 1fr)); gap: 10px; }
    .stat-card { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07); border-radius: 12px; padding: 1rem 0.7rem; text-align: center; }
    ...
    /* rating row inside overview card */
    ...
    .see-reviews-link { font-size: 12px; color: #60a5fa; cursor: pointer; text-decoration: underline; text-underline-offset: 2px; background: none; border: none; font-family: inherit; padding: 0; transition: color 0.2s; }
    ...
    .feat-tag { background: rgba(37,99,235,0.18); color: #a8c4f8; border: 1px solid rgba(100,150,255,0.18); font-size: 12px; padding: 6px 14px; border-radius: 6px; }
    ...
    /* partner card */
    .partner-info { display: flex; align-items: center; gap: 16px; background: rgba(201,162,39,0.06); border: 1px solid rgba(201,162,39,0.18); border-radius: 12px; padding: 1rem 1.1rem; }
    .partner-avatar { width: 48px; height: 48px; border-radius: 50%; flex-shrink: 0; background: rgba(201,162,39,0.20); border: 1px solid rgba(201,162,39,0.30); display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: 700; color: #c9a227; font-family: 'Playfair Display', serif; }
    .partner-name-row { font-weight: 600; color: #fff; font-size: 14px; margin-bottom: 2px; }
    .partner-sub-row  { font-size: 11px; color: rgba(255,255,255,0.35); }
    .partner-badge-inline { margin-left: auto; flex-shrink: 0; background: rgba(201,162,39,0.15); color: #c9a227; border: 1px solid rgba(201,162,39,0.25); font-size: 9px; font-weight: 800; letter-spacing: 1.5px; padding: 4px 10px; border-radius: 20px; text-transform: uppercase; }

    /* ══ SIDEBAR ══ */
    .sidebar-wrap { position: sticky; top: 2rem; }
    .sidebar-card { background: #12151f; border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; overflow: hidden; margin-bottom: 1rem; }
    .price-head { background: linear-gradient(135deg, #1a1f35 0%, #0d1020 100%); border-bottom: 1px solid rgba(255,255,255,0.07); padding: 1.6rem 1.5rem; }
    .price-from-lbl { font-size: 10px; font-weight: 700; color: rgba(255,255,255,0.32); text-transform: uppercase; letter-spacing: 0.10em; margin-bottom: 7px; }
    ...
    .s-row { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 13px; }
    .s-row:last-of-type { border-bottom: none; }
    .s-row .lbl { color: rgba(255,255,255,0.35); }
    .s-row .val { color: #fff; font-weight: 500; }
    .expiry-countdown { margin-top: 14px; padding: 10px 12px; border-radius: 8px; display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 500; }
    .expiry-countdown.ok   { background: rgba(76,175,125,0.10); color: #4caf7d; border: 1px solid rgba(76,175,125,0.20); }
    .expiry-countdown.warn { background: rgba(240,160,48,0.12); color: #f0a030; border: 1px solid rgba(240,160,48,0.25); }
    .expiry-countdown.hot  { background: rgba(224,85,85,0.12);  color: #e05555; border: 1px solid rgba(224,85,85,0.25); }
    .book-btn { display: block; width: 100%; margin-top: 1.2rem; background: #2563eb; color: #fff; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 600; border: none; border-radius: 12px; padding: 15px; cursor: pointer; text-align: center; text-decoration: none; letter-spacing: 0.04em; transition: background 0.15s, transform 0.1s; }
    .book-btn:hover  { background: #1d4ed8; }
    .book-btn:active { transform: scale(0.98); }
    .contact-btn { display: block; width: 100%; margin-top: 10px; background: transparent; border: 1px solid rgba(201,162,39,0.35); color: #c9a227; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; border-radius: 12px; padding: 13px; cursor: pointer; text-align: center; text-decoration: none; transition: background 0.15s, color 0.15s, border-color 0.15s; }
    .contact-btn:hover { background: rgba(201,162,39,0.12); border-color: #c9a227; color: #e8c44a; }
    .wish-btn { display: flex; align-items: center; justify-content: center; gap: 7px; width: 100%; margin-top: 10px; background: transparent; border: 1px solid rgba(255,255,255,0.10); color: rgba(255,255,255,0.40); font-family: 'DM Sans', sans-serif; font-size: 13px; border-radius: 12px; padding: 13px; cursor: pointer; transition: border-color 0.2s, color 0.2s; }
    .wish-btn:hover { border-color: #e84393; color: #e84393; }
    .share-row { display: flex; gap: 8px; margin-top: 10px; }
    .share-btn { flex: 1; display: flex; align-items: center; justify-content: center; gap: 5px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); color: rgba(255,255,255,0.45); font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; border-radius: 10px; padding: 10px 6px; cursor: pointer; transition: background 0.15s, color 0.15s; }
    .share-btn:hover { background: rgba(255,255,255,0.10); color: #fff; }
    .disclaimer { margin-top: 12px; padding: 12px 14px; border-radius: 10px; background: rgba(201,162,39,0.05); border: 1px solid rgba(201,162,39,0.12); font-size: 11px; color: rgba(255,255,255,0.30); line-height: 1.6; }
    .disclaimer strong { color: rgba(201,162,39,0.7); }
    ...
    .reviewer-avatar { width: 46px; height: 46px; border-radius: 50%; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 17px; font-weight: 700; color: #fff; border: 2px solid rgba(255,255,255,0.10); overflow: hidden; }
...
<div class="page-overlay">
  ...
  <div class="detail-hero">
    ...
    <div class="hero-top">
      ...
      <div class="hero-badges">
        <?php if (!empty($deal['category'])): ?><span class="badge-cat"><?= htmlspecialchars($deal['category']) ?></span><?php endif; ?>
        <?php if ($discount > 0): ?><span class="badge-disc">-<?= $discount ?>% OFF</span><?php endif; ?>
        <span class="badge-partner">✦ Partner Listing</span>
      </div>
    </div>
    <div class="hero-bottom">
      <p class="hero-eyebrow">Partner Deal &amp; Package</p>
      <h1 class="hero-title"><?= htmlspecialchars($deal['title']) ?></h1>
      <div class="hero-meta">
        <?php if (!empty($deal['location'])): ?><span class="hero-meta-item">📍 <?= htmlspecialchars($deal['location']) ?></span><?php endif; ?>
        <?php if (!empty($deal['days'])): ?><span class="hero-meta-item">📅 <?= (int)$deal['days'] ?> day<?= (int)$deal['days'] > 1 ? 's' : '' ?></span><?php endif; ?>
        <?php if (!empty($deal['season'])): ?><span class="hero-meta-item">🗓 <?= htmlspecialchars($deal['season']) ?></span><?php endif; ?>
        <?php if ($avg_rating > 0): ?>
          <span class="hero-meta-item">
            <span class="rating-clickable" onclick="openReviews()" title="See all reviews">
              ...
              <?php if ($total_reviews > 0): ?>
                <span style="color:rgba(255,255,255,0.40);font-size:11px">(<?= $total_reviews ?> review<?= $total_reviews !== 1 ? 's' : '' ?>)</span>
              <?php endif; ?>
        ...
        <?php if ($daysLeft !== null): ?>
          <?php $expClass = $daysLeft <= 3 ? 'expiry-hot' : ($daysLeft <= 7 ? 'expiry-warn' : 'expiry-ok'); $expIcon = $daysLeft <= 3 ? '🔥' : ($daysLeft <= 7 ? '⏱' : '✅'); ?>
          <span class="hero-meta-item">
            <span class="expiry-badge-meta <?= $expClass ?>">
              <?= $expIcon ?>
              <?php if ($daysLeft === 0): ?>Expires today!
              <?php elseif ($daysLeft <= 3): ?><?= $daysLeft ?>d left — book fast!
              <?php else: ?><?= $daysLeft ?> days left<?php endif; ?>
            </span>
          </span>
        <?php endif; ?>
      </div>
  ...
  <div class="detail-layout">
    ...
    <div>
      <!-- Overview card with rating stat -->
      <div class="glass-card">
        <p class="sec-label">Overview</p>
        <div class="stat-grid">
          <?php if (!empty($deal['days'])): ?>
          <div class="stat-card"><div class="stat-icon">📅</div><div class="stat-val"><?= (int)$deal['days'] ?></div><div class="stat-lbl">Days</div></div>
          <?php endif; ?>
          <?php if ($avg_rating > 0): ?>
          ...
          <div class="stat-card"><div class="stat-icon">📍</div><div class="stat-val" style="font-size:0.80rem"><?= htmlspecialchars($deal['location']) ?></div><div class="stat-lbl">Location</div></div>
          <?php endif; ?>
          <?php if (!empty($deal['season'])): ?>
          <div class="stat-card"><div class="stat-icon">🗓</div><div class="stat-val" style="font-size:0.80rem"><?= htmlspecialchars($deal['season']) ?></div><div class="stat-lbl">Season</div></div>
          <?php endif; ?>
          <?php if ($daysLeft !== null): ?>
          <div class="stat-card" style="<?= $daysLeft <= 3 ? 'border-color:rgba(224,85,85,0.30)' : ($daysLeft <= 7 ? 'border-color:rgba(240,160,48,0.25)' : '') ?>">
            <div class="stat-icon"><?= $daysLeft <= 3 ? '🔥' : ($daysLeft <= 7 ? '⏱' : '✅') ?></div>
            <div class="stat-val" style="<?= $daysLeft <= 3 ? 'color:#e05555' : ($daysLeft <= 7 ? 'color:#f0a030' : '') ?>"><?= $daysLeft ?>d</div>
        ...
        <div class="rating-row">
          ...
          <?php if ($total_reviews > 0): ?>
            <span class="review-cnt">(<?= $total_reviews ?> review<?= $total_reviews !== 1 ? 's' : '' ?>)</span>
        ...
        <table class="info-tbl">
          ...
          <tr>
            ...
            <td class="val">⭐ <?= number_format($avg_rating, 1) ?> / 5
              <?php if ($total_reviews > 0): ?>
                <button class="see-reviews-link" onclick="openReviews()" style="margin-left:6px">(<?= $total_reviews ?> review<?= $total_reviews !== 1 ? 's' : '' ?>)</button>
              <?php endif; ?>
            </td>
          </tr>
          <?php endif; ?>
          <?php if (!empty($deal['visible_until'])): ?>
          <tr>
            <td class="lbl">Available Until</td>
            <td class="val">
              <?= date('F j, Y', strtotime($deal['visible_until'])) ?>
              <?php if ($daysLeft !== null && $daysLeft <= 7): ?>
                <span style="margin-left:8px;font-size:11px;color:<?= $daysLeft <= 3 ? '#e05555' : '#f0a030' ?>;font-weight:600">(<?= $daysLeft ?>d left)</span>
              <?php endif; ?>
            </td>
          </tr>
          <?php endif; ?>
          <?php if (!empty($deal['plan_name'])): ?><tr><td class="lbl">Listing Tier</td><td class="val" style="color:#c9a227">✦ <?= htmlspecialchars($deal['plan_name']) ?></td></tr><?php endif; ?>
          <?php if (!empty($deal['submitted_by'])): ?><tr><td class="lbl">Listed By</td><td class="val"><?= htmlspecialchars($deal['submitted_by']) ?></td></tr><?php endif; ?>
      ...
      <?php if (!empty($deal['submitted_by'])): ?>
      <div class="glass-card partner-highlight">
        <p class="sec-label">Listed By</p>
        <div class="partner-info">
          <div class="partner-avatar"><?= strtoupper(substr($deal['submitted_by'], 0, 1)) ?></div>
          <div>
            <div class="partner-name-row"><?= htmlspecialchars($deal['submitted_by']) ?></div>
            <?php if (!empty($deal['plan_name'])): ?><div class="partner-sub-row">Subscription: <?= htmlspecialchars($deal['plan_name']) ?></div><?php endif; ?>
            <?php if (!empty($deal['submitted_email'])): ?><div class="partner-sub-row" style="margin-top:3px"><?= htmlspecialchars($deal['submitted_email']) ?></div><?php endif; ?>
          </div>
          <span class="partner-badge-inline">Verified Partner</span>
        </div>
      </div>
      <?php endif; ?>
    </div>
    ...
    <div class="sidebar-wrap">
      <div class="sidebar-card">
        ...
        <div class="s-body">
          ...
          <?php endif; ?>
          <?php if (!empty($deal['submitted_by'])): ?><div class="s-row"><span class="lbl">Partner</span><span class="val" style="color:#c9a227">✦ <?= htmlspecialchars($deal['submitted_by']) ?></span></div><?php endif; ?>

          <?php if ($daysLeft !== null): ?>
          <div class="expiry-countdown <?= $daysLeft <= 3 ? 'hot' : ($daysLeft <= 7 ? 'warn' : 'ok') ?>">
            <?= $daysLeft <= 3 ? '🔥' : ($daysLeft <= 7 ? '⏱' : '✅') ?>
            <?php if ($daysLeft === 0): ?>
              <span>Expires <strong>today</strong> — book now!</span>
            <?php elseif ($daysLeft <= 3): ?>
              <span>Only <strong><?= $daysLeft ?> day<?= $daysLeft > 1 ? 's' : '' ?></strong> left!</span>
            <?php else: ?>
              <span>Available for <strong><?= $daysLeft ?> more days</strong></span>
            <?php endif; ?>
          </div>
          <?php endif; ?>

          <a href="booking.php?ud=<?= (int)$ud_id ?>" class="book-btn">Book Now</a>

          <?php if (!empty($deal['submitted_email'])): ?>
          <a href="mailto:<?= htmlspecialchars($deal['submitted_email']) ?>?subject=<?= urlencode('Enquiry: ' . $deal['title']) ?>" class="contact-btn">✉ Contact Partner</a>
          <?php endif; ?>

          <button class="wish-btn" onclick="window.location.href='save_deal.php?ud=<?= (int)$ud_id ?>&action=add'">♡ &nbsp; Save to Wishlist</button>
          ...
          <div class="disclaimer">
            <strong>Partner Listing</strong> — This deal is offered by a verified Nepal Travel partner. Nepal Travel facilitates the listing but does not operate this package directly.
          </div>
        </div>
      </div>
    </div>
