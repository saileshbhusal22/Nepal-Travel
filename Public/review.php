...
if ($booking_id <= 0) { header("Location: deals.php"); exit; }

// Load booking + deal + user using deal_id FK
$stmt = $conn->prepare("
    SELECT 
        b.id          AS booking_id,
...
if (!$row) { header("Location: deals.php"); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$existing) {
    ...
    if ($rating < 1 || $rating > 5) {
        ...
        try {
            // Insert the review
            $stmt = $conn->prepare("
                INSERT INTO deal_reviews (deal_id, booking_id, user_id, rating, review_text)
                VALUES (?, ?, ?, ?, ?)
            ");
            if (!$stmt) {
            ...
            $stmt->bind_param("iiiis", $deal_id, $booking_id, $user_id, $rating, $review_text);
            ...
            // Check if average_rating column exists in deals table
            $check_col = $conn->query("SHOW COLUMNS FROM deals LIKE 'average_rating'");
            if ($check_col->num_rows > 0) {
                // Update the deals table - update average_rating
                $avg_stmt = $conn->prepare("
                    SELECT AVG(rating) as avg_rating, COUNT(*) as review_count 
                    FROM deal_reviews 
                    WHERE deal_id = ?
                ");
                $avg_stmt->bind_param("i", $deal_id);
                $avg_stmt->execute();
                $avg_result = $avg_stmt->get_result()->fetch_assoc();
                $avg_stmt->close();
                
                $new_avg_rating = round($avg_result['avg_rating'], 1);
                ...
                $update_stmt = $conn->prepare("
                    ...
                    WHERE id = ?
                ");
                $update_stmt->bind_param("di", $new_avg_rating, $deal_id);
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      font-family: 'DM Sans', sans-serif; min-height: 100vh; color: #fff;
    ...
    .deal-banner-text h2 {
      font-family: 'Playfair Display', serif; font-size: 1.25rem; color: #fff;
    }
    .deal-banner-meta {
    ...
    .card-title {
      ...
      font-size: 1.5rem; color: #fff; margin-bottom: 0.3rem;
    }
    ...
    .star-hint {
      ...
      margin-top: 6px; min-height: 18px; transition: color 0.2s;
    }
    ...
    .success-title {
      font-family: 'Playfair Display', serif; font-size: 1.9rem; color: #fff; margin-bottom: 0.6rem;
    }
...
<div class="success-wrap">
  ...
  <div class="success-stars"><?= str_repeat('★', (int)$_POST['rating']) ?><?= str_repeat('☆', 5 - (int)$_POST['rating']) ?></div>
  <p class="success-msg">
    Your review for <strong style="color:rgba(255,255,255,0.70)"><?= htmlspecialchars($row['deal_title']) ?></strong>
    has been saved and will help other travellers plan their adventure.
  </p>
  <a href="deal-detail.php?id=<?= (int)$row['deal_id'] ?>" class="go-btn">Back to Deal →</a>
</div>
...
<div class="already-wrap">
  ...
  <p>You've already reviewed <strong style="color:rgba(255,255,255,0.65)"><?= htmlspecialchars($row['deal_title']) ?></strong>.</p>
  <div class="your-review">
    <div class="your-stars"><?= str_repeat('★', (int)$existing['rating']) ?><?= str_repeat('☆', 5 - (int)$existing['rating']) ?></div>
    <p class="your-text"><?= nl2br(htmlspecialchars($existing['review_text'])) ?></p>
  </div>
  <a href="deal-detail.php?id=<?= (int)$row['deal_id'] ?>" style="color:#a8c4f8; font-size:13px;">View the deal →</a>
</div>
...
<div class="card-body">
  ...
  <a href="deal-detail.php?id=<?= (int)$row['deal_id'] ?>" class="skip-link">Maybe later →</a>
</div>
...
<script>
  ...
  document.querySelectorAll('.star-picker label').forEach(label => {
    label.addEventListener('mouseenter', () => {
      const forVal = label.getAttribute('for').replace('s','');
      if (hint) hint.textContent = hints[forVal] ?? '';
    });
  });
