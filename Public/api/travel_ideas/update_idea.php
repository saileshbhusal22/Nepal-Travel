...
error_reporting(E_ALL); include "debug_logger.php";

header('Content-Type: application/json; charset=utf-8');

set_exception_handler(function ($exception) {
    file_put_contents(__DIR__ . '/debug.log', "EXCEPTION: " . $exception->getMessage() . "\n" . $exception->getTraceAsString() . "\n\n", FILE_APPEND);
...
register_shutdown_function(function () {
    ...
    if ($error && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_COMPILE_WARNING])) {
        file_put_contents(__DIR__ . '/debug.log', "FATAL ERROR: " . print_r($error, true) . "\n\n", FILE_APPEND);
...
$idea_id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
if ($idea_id <= 0) {
...
$chk->bind_param("ii", $idea_id, $user_id);
$chk->execute();
if ($chk->get_result()->num_rows === 0) {
    echo json_encode(['success' => false, 'message' => 'Not authorized to update this idea']);
    exit;
}
$chk->close();
...
if (empty($slug)) {
    ...
    $counter = 1;
    while (true) {
        $checkStmt = $conn->prepare("SELECT id FROM travel_ideas WHERE slug = ? AND id != ? LIMIT 1");
        if ($checkStmt) {
            $checkStmt->bind_param('si', $slug, $idea_id);
            $checkStmt->execute();
...
$updateSql = "UPDATE travel_ideas SET title=?, subtitle=?, slug=?, province_id=?, province_slug=?, duration_days=?, nights=?, transport=?, accommodation=?, best_time=?, pro_tip=?, difficulty=?";
$bindTypes = "sssisiiissss";
$bindArgs = [&$title, &$subtitle_value, &$slug, &$province_id, &$province_slug_value, &$duration_days_value, &$nights_value, &$transport_value, &$accommodation_value, &$best_time_value, &$pro_tip_value, &$difficulty_value];
...
if ($stmt->execute()) {
    ...
    $dstmt = $conn->prepare("UPDATE travel_idea_details SET content=?, highlights=?, logistics=?".(!empty($image_path)?", hero_image=?":"")." WHERE idea_id=?");
    if ($dstmt) {
        if (!empty($image_path)) {
            $dstmt->bind_param("ssssi", $intro, $highlights, $logistics, $image_path, $idea_id);
        } else {
            $dstmt->bind_param("sssi", $intro, $highlights, $logistics, $idea_id);
        }
        $dstmt->execute();
        $dstmt->close();
    }

    $conn->query("DELETE FROM travel_idea_experiences WHERE idea_id = " . (int)$idea_id);
    ...
    $conn->query("DELETE FROM itineraries WHERE idea_id = " . (int)$idea_id);
    ...
    echo json_encode(['success'=>true,'message'=>'Travel idea updated','slug'=>$slug]);
} else {
