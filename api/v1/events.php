...
// ---------------------------------------------------------
    ...
    $userRes = $conn->query("SELECT is_subscribed FROM users WHERE id = $user_id");
    $userData = $userRes->fetch_assoc();
    $isSubscribed = (isset($userData['is_subscribed']) && $userData['is_subscribed'] == 1);

    /* 
    if (!$isAdmin && !$isSubscribed) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Subscription required to post events']);
        exit;
    }
    */
    ...
    // Handle Image Upload
    $image_path = 'images/placeholder_event.jpg';
    ...
    if (isset($_FILES['gallery_images'])) {
        ...
        for ($i = 0; $i < $count; $i++) {
            if ($fileArray['error'][$i] === UPLOAD_ERR_OK) {
                $fileName = time() . '_gallery_' . $i . '_' . basename($fileArray['name'][$i]);
                $targetPath = $uploadDir . $fileName;
                if (move_uploaded_file($fileArray['tmp_name'][$i], $targetPath)) {
                    $galleryPaths[] = 'images/uploads/' . $fileName;
                }
            }
        }
    }
    
    // If no new gallery images were uploaded, try to keep the existing ones
    if (empty($galleryPaths) && !empty($_POST['gallery_images'])) {
        $gallery_images = $_POST['gallery_images'];
    } else {
        $gallery_images = !empty($galleryPaths) ? implode(',', $galleryPaths) : "";
    }
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        ...
        $types = str_repeat('s', 18) . 'isss' . 'idiisi' . str_repeat('s', 6) . 'iiii' . str_repeat('s', 5) . 'ii' . str_repeat('s', 3) . 'd' . 'ii';
        $stmt->bind_param($types, 
            $title, $tags, $language, $age_group, $description, $what_to_expect, $image_path, $gallery_images,
            $category, $event_date, $month, $location, $full_address, $region,
            $start_date, $end_date, $start_time, $end_time, $is_recurring, $recurring_frequency,
        ...
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Event processed successfully', 'id' => $conn->insert_id ?: ($_POST['id'] ?? 0)]);
        } else {
